...
const char Version[] = "versao";
const char InvalidCommand[] = "Comando invalido!";

// XBee Network
const char AutoConfigure[] = "auto_configuracao";
const char ConfigureSlave[] = "configuracao_slave";
const char Restore[] = "restaurar";
...
const char LNetwork_serverPort[] = "porta_servidor";
const char LNetwork_clientPort[] = "porta_cliente";
...
//    digitalWrite(pinERROR, HIGH); // for SMD version *** MODIFY
  ...
  server.addCallback("/ARP/test", &TEST); //RoboCore callback to verify calls - for testing
  server.addCallback("/ARP/configure/^", &Configure); //configure master|slave >> currently not very useful
  server.addCallback("/ARP/IR/record", &IRRecord); // record the IR command
  server.addCallback("/ARP/IR/enviar/^", &IRSend); //send/save the IR command
  server.addCallback("/ARP/rfid/add/^", &RFIDAdd); //add an rfid to the access list
  server.addCallback("/ARP/rfid/remove/^", &RFIDRemove); //remove an rfid from the access list
...
//  digitalWrite(pinON, HIGH); // *** MODIFY
  Serial.println("--- Automacao Residencial Modular ---");
...
void loop(){
  ...
  if(Serial.available()){
    ...
    if((length == StrLength((char*)AutoConfigure)) && (StrCompare(temp, (char*)AutoConfigure, (byte)CASE_INSENSITIVE) == StrLength((char*)AutoConfigure))){ //must be exact match
      Serial.println();
      Serial.println("# Auto Configuracao do Master");
      //try common values
      Serial.println("9600");
      ...
      //display result
      if(res != 1){
        Serial.println("ERRO na configuracao");
      } else {
    ...
    //configure as slave
      ...
      if((StrLength(config) == StrLength((char*)ConfigureSlave)) && (StrCompare(config, (char*)ConfigureSlave, (byte)CASE_INSENSITIVE) == StrLength((char*)ConfigureSlave))){ //must be exact match
        ...
        if(SlaveTypeExists(slave_type)){
          ...
          //display result
          if(res != 2){
            Serial.println("ERRO na configuraçao");
          } else {
            Serial.println("feito!");
          }
        }
    ...
    //restore values
      ...
      if(res != 1){
        ...
        Serial.println("Ajustes de fabrica restaurados!");
      }
    }
    //display version
